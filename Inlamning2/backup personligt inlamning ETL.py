...
print(merging_files)
#merging_files_2=pd.merge(merging_files, df3, on = 'Enhet', how = 'outer')
#print(merging_files_2)

# #byta namn till kolumner och spara enbart de mest relevanta - drop allt annat:

cleaned_data =merging_files.drop(['Kategori_x', 'Kategori_y', 'Månad_x','Månad_y', 'År_y','År_x'], axis=1)
cleaned_data_2 = cleaned_data.rename(columns={'Kökssvinn(i kg)_x':'januari(i kg)', 'Kökssvinn(i kg)_y':'februari(i kg)'})

# cleaned_data =merging_files_2.drop(['Kategori_x', 'Kategori_y', 'Kategori', 'Månad_x', 'Månad_y', 'Månad', 'År','År_x', 'År_y'], axis=1)
# cleaned_data_2 = cleaned_data.rename(columns={'Kökssvinn(i kg)_x':'januari(i kg)', 'Kökssvinn(i kg)_y':'februari(i kg)', 'Kökssvinn(i kg)': 'mars(i kg)'})

print(cleaned_data_2)

#61 raderkvar inkl. kolumnnamn


# #jag vill visa jamforelse over tid under tva manader. Da tar jag bort de raderna dar minst det ena vardet varde ar 'NaN':

cleaning_data_3 = cleaned_data_2.dropna(subset=['januari(i kg)', 'februari(i kg)'])

#cleaning_data_3 = cleaned_data_2.dropna(subset=['januari(i kg)', 'februari(i kg)', 'mars(i kg)'])
print(cleaning_data_3)
#__________40 rader kvar inkl. kolumnnamn


'''#kollar hur manga kolumner jag har:
rows = len(cleaning_data_3)
print(rows)'''

#skapar nya kolumner:

#1. raknar skillnad i % av matsvinn fran januari till februari:
cleaning_data_3['Matsvinn jan-feb i %']= (cleaning_data_3['februari(i kg)'] - cleaning_data_3['januari(i kg)'])/cleaning_data_3['januari(i kg)'] * 100
cleaning_data_3['Matsvinn jan-feb i %'] = cleaning_data_3['Matsvinn jan-feb i %'].round(1).astype(int) 


# # cleaning_data_3['Matsvinn feb_mars i %']= (cleaning_data_3['mars(i kg)'] - cleaning_data_3['februari(i kg)'])/cleaning_data_3['februari(i kg)'] * 100
# # cleaning_data_3['Matsvinn feb_mars i %'] = cleaning_data_3['Matsvinn feb_mars i %'].round(1).astype(int) 

# # cleaning_data_3['Medel %']=(cleaning_data_3['Matsvinn feb_mars i %'] + cleaning_data_3['Matsvinn jan-feb i %'])/2
# # cleaning_data_3['Medel %'] = cleaning_data_3['Medel %'].astype(int) 

print(cleaning_data_3)


#sorting_more: forskolor x geografisk omrade (hittade info pa Sodertalje kommun hemsida)
#Brunnsang - Grusasen omrade (BG):
BG= ['Algården',  'Grusåsen', 'Bergvik' 'Oxelgrenshagen', 'Trädgården']
BG_omrade= cleaning_data_3[cleaning_data_3['Enhet'].isin(BG)]
print(BG_omrade)


#Centrum omrade(Centrum)
Centrum = ['Badparkens', 'Blombacka', 'Braxen', 'Filen', 'Oxbacken', 'Vattentornet']
Centrum_omrade= cleaning_data_3[cleaning_data_3['Enhet'].isin(Centrum)]
print(Centrum_omrade)


#Mariekäll-Saltskog(MS)
MS = ['Annexet', 'Burspråket', 'Fasaden', 'Petunian', 'Taket', 'Gullpudran']
MS_omrade= cleaning_data_3[cleaning_data_3['Enhet'].isin(MS)]
print(MS_omrade)

#Lina-Enhörna (LE)
LE = ['Eken', 'Valla', 'Lina', 'Kaxberg']
LE_omrade= cleaning_data_3[cleaning_data_3['Enhet'].isin(LE)]
print(LE_omrade)

#Ritorp - Viksberg (RV)
RV=['Björkäng', 'Sommaräng', 'Viksberg', 'Årsboken']
RV_omrade=cleaning_data_3[cleaning_data_3['Enhet'].isin(RV)]
print(RV_omrade)

#Rosenlund-Fornhöjden (RF)
RF=['Körsbäret', 'Morellen', 'Växthuset', 'Pärlan', 'Rubinen']
RF_omrade=cleaning_data_3[cleaning_data_3['Enhet'].isin(RF)]
print(RF_omrade)

#Tveta (TV)
TV=['Saltsjön', 'Vikingen', 'Hovsjö', 'Granö', 'Måsen']
Tv_omrade=cleaning_data_3[cleaning_data_3['Enhet'].isin(TV)]
print(Tv_omrade)

#Västra (VST)
VST=['Blommans', 'Brolunda', 'Trossens', 'Skeppet', 'Svingeln', 'Högmohöjdens']
VST_omrade=cleaning_data_3[cleaning_data_3['Enhet'].isin(VST)]
print(VST_omrade)

#Ostertalje (OST)
OST= ['Glasberga', 'Hästhoven', 'Solhem', 'Tygeln']
OST_omrade=cleaning_data_3[cleaning_data_3['Enhet'].isin(OST)]
print(OST_omrade)

#andra forskolor (annat)

cleaning_data_3['Location']: None
cleaning_data_3.loc[cleaning_data_3['Enhet'].isin(VST_omrade), 'Location'] = 'VST'
